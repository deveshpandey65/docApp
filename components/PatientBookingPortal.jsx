...
const PatientBookingPortal = () => {
  ...
  const fetchDoctorSchedule = async (doctorId, date) => {
    try {
      setLoading(true);
      const response = await fetch(`${apiUrl}/schedule?doctorId=${doctorId}&date=${date}`, {
        headers: {
  ...
  const requestSlot = async (scheduleId, slotIndex, slotFee) => {
    try {
      ...
      const response = await fetch(`${apiUrl}/slots/request`, {
        method: 'POST',
        headers: {
  ...
  const createRazorpayOrder = async (slotRequestId, amount) => {
    try {
      const response = await fetch(`${apiUrl}/payments/order`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
