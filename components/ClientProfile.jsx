...
const ClientProfile = () => {
  ...
  useEffect(() => {
    useClientAuthStore.getState().checkAuth();
  }, []);
  ...
  if (isCheckingAuth) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl">Authenticating client...</p>
      </div>
    );
  }
  ...
  if (!client) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl text-red-500">Failed to load client profile. Please log in.</p>
      </div>
    );
  }
