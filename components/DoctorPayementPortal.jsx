...
const DoctorPaymentPortal = () => {
  ...
  const { isAuthenticated, doctor, checkAuth } = useDoctorAuthStore();

  // ✅ 1. Check authentication once on mount
  useEffect(() => {
    checkAuth();
  }, []);

  // ✅ 2. Fetch payment history only after doctor info is available
  useEffect(() => {
    if (isAuthenticated && doctor?._id) {
      fetchPaymentHistory();
    }
  ...
  const fetchPaymentHistory = async () => {
    try {
      ...
      console.log("✅ Fetching history for doctor:", doctor);
      if (!doctor?._id) {
        throw new Error('Doctor information not available');
      }

      const doctorId = doctor._id;
      const response = await fetch(`${apiUrl}/payments/history?doctorId=${doctorId}`);
                    ...
                    <tr key={payment.paymentId} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div>
                            <div className="text-sm font-medium text-gray-900">{payment.client.name}</div>
                            <div className="text-sm text-gray-500">Age: {payment.client.age} years</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
