...
const ClientSignup = () => {
  ...
  const { register, clientOtpRegister, verifyOtp, verifyEmail, isLoading, error, clearError } = useClientAuthStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success && result.clientId) {
        // ✅ Save clientId for later OTP verification
        setClientId(result.clientId);
        localStorage.setItem("clientTempId", result.clientId); // optional

        // ✅ Show OTP modal or navigate
  ...
  const handleVerifyOtp = async () => {
    ...
    clearError(); // if this clears global error
    setStoreError(''); // reset global store error

    try {
      const result = await clientOtpRegister(clientId, otp);
