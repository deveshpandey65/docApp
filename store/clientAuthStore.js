...
const useClientAuthStore = create((set) => ({
  ...
  register: async (formData) => {
    ...
    try {
      ...
      const tempClientId = response.data?.data?.clientId;

      if (tempClientId) {
        localStorage.setItem("clientTempId", tempClientId); // ✅ Save for OTP verification
      }
      ...
      return { success: true, clientId: tempClientId };
    } catch (error) {
  ...
  clientOtpRegister: async (clientId, otp) => {
    set({ isLoading: true, error: null });

    try {
      const response = await axiosInstance.post(
        `${API_URL}/client/register/verify-otp`,
        { clientId, otp },
        { withCredentials: true }
      );
      const { accessToken, refreshToken, client } = response.data;

      // Optional: Store tokens only if not using httpOnly cookies
      ...
      if (client?._id) localStorage.setItem("clientId", client._id);
